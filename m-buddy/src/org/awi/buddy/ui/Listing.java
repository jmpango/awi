...
public class Listing extends BaseActivity implements OnItemClickListener, TextWatcher {
	...
	private class ListingAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			listingListView.setAdapter(new BuddyDataBinder(listing, tempListingList));
		}
	...
	private class ListingSearchAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		@Override
		protected Void doInBackground(Void... params) {
			tempListingList = listingService.searchBuddies(dashBoardSearchBox.getText().toString(), listingList);
			return null;
		}
		...
		protected void onPostExecute(Void result) {
			if(tempListingList.size() == 0){
				...
				listingListView.setAdapter(new BuddyDataBinder(listing, new ArrayList<Buddy>()));
			}else{
				listingListView.setAdapter(new BuddyDataBinder(listing, tempListingList));
			}
		}
